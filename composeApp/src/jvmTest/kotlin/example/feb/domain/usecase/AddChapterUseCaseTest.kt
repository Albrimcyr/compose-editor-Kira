...
class AddChapterUseCaseTest {
    ...
    fun `adds a new chapter and persists it`() = runTest {
        ...
        Assertions.assertTrue(result.isSuccess)                                                // expected success

        val chapter = result.getOrNull()!!                                                     // ADD Chapter
        Assertions.assertEquals(Chapter.DEFAULT_TITLE, chapter.title)       // title
        Assertions.assertEquals("", chapter.content)                        // blank
        Assertions.assertEquals(1, repo.upsertCalls)                        // how many times called?
        Assertions.assertEquals(chapter, repo.get(chapter.id))              // actually in storage?
    }
    ...
    fun `two chapters = two different IDs`() = runTest {
        ...
        Assertions.assertNotEquals(a.id, b.id)               // different?
        Assertions.assertEquals(2, repo.upsertCalls)           // how many times called?
    }
